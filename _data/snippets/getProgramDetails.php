...
$tpl = $modx->getOption('tpl', $scriptProperties, 'hero-1');
$episodeTpl = $modx->getOption('episodeTpl', $scriptProperties, 'episodeTpl');
...
$speakers ='';
foreach ($data['speakers']['data'] as $speaker) {
    $speakers .= $speaker['attributes']['fullname']. ', ';
}
$categories ='';
foreach ($data['categories']['data'] as $cat) {
    $categories .= $cat['attributes']['category_name']. ', ';
}
$episodes ='';
foreach ($data['episodes'] as $episode) {
    $episodes .= $modx-> getChunk($episodeTpl, [
        'thumbnailUrl' => $coverUrl,
        'title' => $episode['ep_title'],
        'epNumber' => $episode['ep_number'],
        'flowplayerId' => $episode['flowplayer_id'],
        'published' => $episode['publish'],
        'epUrl' => 'https://sumtv.app/programs/'.$resource->getTVValue('strapiId').'/episode/'.$episode['ep_number'],
    ]);
...
$placeholders = array_merge($data, [
    ...
    'serieUrl'     => 'https://sumtv.app/programs/' . $resource->getTVValue('strapiId'),
    'episodes'     => $episodes,
    'serieDescription' => $data['serie_desc'],
]);
